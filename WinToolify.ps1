...
$script:Translations = @{
    "EN" = @{
        ...
        "BackUpInfo3" = "you use before resetting."
        "InstallSelectedPrograms" = "Install The Selected Programs"
        "SelectedProgramsInstalled" = "Selected programs has been installed."
        "SelectProgramsFirst" = "Please select apps to install first."
        "BackUpCreated" = "The Backup folder on the desktop was successfully created."
        "DocumantaryBaced" = "Documents and Desktop Files Backed."
        "LicenseRecorded" = "License information was recorded."
        "ProgramsListed" = "Registered programs in Windows are listed."
        "ProgramsListed2" = "Program List was created."
        "WingetError" = "Winget is not installed."
        "WingetError2" = "To install Winget, you can install 'App Installer' application from Microsoft Store."
        "BackUpNotFound" = "Error: The Backup folder was not found on the desktop. You must make a backup first."
        "InstalledProgramsNotFound" = "Error: The program list (InstalledPrograms.txt) not found. You must make a backup first."
        "Installing" = "Selected program is loaded."
    ...
    "TR" = @{
        ...
        "BackUpInfo3" = "programlarinizi yükleyebilirsiniz."
        "InstallSelectedPrograms" = "Seçilen Programlari Yükleyin."
        "SelectedProgramsInstalled" = "Secili uygulamalar yüklendi."
        "SelectProgramsFirst" = "Lutfen once indirilecek uygulamalari secin."
        "BackUpCreated" = "Masaüstünde backup klasörü başariyla oluşturuldu." 
        "DocumantaryBaced" = "Belgeler ve masaüstü dosyalari yedeklendi."
        "LicenseRecorded" = "Lisans bilgileri kaydedildi."
        "ProgramsListed" = "Windows'ta kayitli programlar listeleniyor."
        "ProgramsListed2" = "Program listesi olusturuldu."
        "WingetError" = "Winget yüklü değil."
        "WingetError2" = "Winget'i yüklemek için Microsoft Store'dan 'App Installer' uygulamasini yükleyebilirsiniz."
        "BackUpNotFound" = "Hata: Yedekleme klasörü masaüstünde bulunamadi. Önce bir yedekleme yapmaniz gerekli."
        "InstalledProgramsNotFound" = "HATA: Program listesi (InstalledPrograms.txt) bulunamadi. Önce yedekleme yapmalisiniz."
        "Installing" = "Secilen uygulama yükleniyor."
        "Installing2" = "Yükleniyor"
...
function Show-LanguageMenu {
    ...
    Write-Host ("$padding|  [M] " + $(Get-Translation 'ReturnMainMenu')).PadRight($padding.Length + 38) "|" -ForegroundColor White
    Write-Host "$padding+--------------------------------------+" -ForegroundColor Yellow
    Write-Host "`n"
...
function Show-Menu {
    ...
    Write-Host ("$padding|  [6] " + $(Get-Translation 'BackUp')).PadRight($padding.Length + 38) "|" -ForegroundColor White
    Write-Host ("$padding|  [7] " + $(Get-Translation 'Language')).PadRight($padding.Length + 38) "|" -ForegroundColor White
    Write-Host  "$padding|                                      |" -ForegroundColor White
...
function Show-ActionToolsMenu {
    ...
    Write-Host  "$padding|                                      |" -ForegroundColor White
    Write-Host ("$padding|  [M] " + $(Get-Translation 'ReturnBasicToolsMenu')).PadRight($padding.Length + 38) "|" -ForegroundColor Red
    Write-Host "$padding+--------------------------------------+" -ForegroundColor Yellow
    Write-Host "`n"
    Write-Host "$padding$(Get-Translation 'SelectOption'): " -NoNewline -ForegroundColor Green
}
...
function Show-BackupMenu {
    ...
    Write-Host "`n"
    Write-Host "$padding+--------------------------------------+" -ForegroundColor Yellow
    Write-Host ("$padding| " + $(Get-Translation 'BackUpInfo')).PadRight($padding.Length + 38) "|" -ForegroundColor White
...
function Remove-SelectedBloatware {
    ...
    do {
        ...
        $Script:choice = Read-Host "`n" $padding + (Get-Translation 'YourChoice')
        ...
        switch ($choice) {
            ...
            "Q" { break }  # Döngüden çıkmak için break kullanın
            default {
                if ($choice -match "^\d+$") {
                    $num = [int]$choice
...
function Test-WingetInstalled {
    try {
        ...
        if (-not (Get-Command winget -or Get-Command Get-WinGetPackage -ErrorAction SilentlyContinue)) {
            Write-Host (Get-Translation 'WingetNotInstalled') -ForegroundColor Yellow
            
            # Winget kurulumunu başlat
            Install-PackageProvider -Name NuGet -Force | Out-Null
            Install-Module -Name Microsoft.WinGet.Client -Force -Repository PSGallery | Out-Null
            Import-Module -Name Microsoft.WinGet.Client |Out-Null
...
function backup {
    #Masaüstü yolunu al ve backup klasörünü oluştur
    $userDesktop = [Environment]::GetFolderPath([Environment+SpecialFolder]::Desktop)
    $Script:backupFolder = Join-Path $userDesktop "Backup"
    
    if (-not (Test-Path $Script:backupFolder)) {
        $folder = New-Item -ItemType Directory -Path $Script:backupFolder
        Write-Host "`n"
        Write-Host (Get-Translation 'BackUpCreated')`n -ForegroundColor Green
    }

    #Masaüsütü,Belgeler, Resimler ve Videoları Backup dosyasına kopyalama
    #Eğer Backup.ps1 kopyalanacak klasörlerden biri içinde çalışıyorsa hata verecektir. Bunu önlemek için hariç tutuyoruz.
    $excludeItems = @("Backup.ps1","Backup")

    $userVideos   = [Environment]::GetFolderPath([Environment+SpecialFolder]::MyVideos)
    $userPictures = [Environment]::GetFolderPath([Environment+SpecialFolder]::MyPictures)
    $userDocuments = [Environment]::GetFolderPath([Environment+SpecialFolder]::MyDocuments)

    # Her klasördeki dosyaları al, hariç tutulanları filtrele ve yedekliyor.
    Get-ChildItem -Path $userVideos -Recurse | Where-Object { $_.Name -notin $excludeItems } | ForEach-Object {
        $destinationPath = Join-Path $Script:backupFolder\Videos $_.FullName.Substring($userVideos.Length)
    ...
    Get-ChildItem -Path $userPictures -Recurse | Where-Object { $_.Name -notin $excludeItems } | ForEach-Object {
        $destinationPath = Join-Path $Script:backupFolder\Pictures $_.FullName.Substring($userPictures.Length)
        $destinationDir = Split-Path -Parent $destinationPath
        if (!(Test-Path $destinationDir)) {
            New-Item -ItemType Directory -Path $destinationDir -Force | Out-Null
        }
    ...
    Get-ChildItem -Path $userDesktop -Recurse | Where-Object { 
        $_.FullName -notlike "$Script:backupFolder*" -and $_.Name -notin $excludeItems 
    } | ForEach-Object {
        $destinationPath = Join-Path $Script:backupFolder\Desktop $_.FullName.Substring($userDesktop.Length)
        $destinationDir = Split-Path -Parent $destinationPath
        if (!(Test-Path $destinationDir)) {
            New-Item -ItemType Directory -Path $destinationDir -Force | Out-Null
        }
    ...
    Write-Host (Get-Translation "DocumantaryBaced")`n -ForegroundColor Green

    #Lisans ve version bilgilerini kaydet
    $licenseFile = "$Script:backupFolder\Licenses.txt"
    "Ürün Lisans Bilgileri" | Out-File -FilePath $licenseFile
    "Windows Lisans Anahtarı: " + (Get-WmiObject -query 'select * from SoftwareLicensingService').OA3xOriginalProductKey | Out-File -FilePath $licenseFile -Append
    Get-WmiObject -Class Win32_OperatingSystem | Select-Object Caption, Version|Out-File -FilePath $licenseFile -Append

    Write-Host (Get-Translation "LicenseRecorded")`n -ForegroundColor Green
    
    #Yüklü güncelleme listesini yedekleme.
    $UpdateFile = "$Script:backupFolder\Update.txt"
    Get-HotFix |Out-File -FilePath $UpdateFile

    #Kurulu programların listesini oluştur
    $programsFile = "$Script:backupFolder\InstalledPrograms.txt"

    # Önce Windows'un kendi program listesini al
    Write-Host (Get-Translation "ProgramsListed") -ForegroundColor Yellow
    try {
        if (Get-Command Get-WinGetPackage -ErrorAction Stop){
            $windowsPrograms = Get-WinGetPackage | Select-Object Name , Id
            $maxLength = 0
            foreach($prog in $windowsPrograms) {
                if ($prog.Name.Length -gt $maxLength) {
                    $maxLength = $prog.Name.Length
                }
            }

            "Program Name" + (" " * ($maxLength - 8)) + "ID" | Out-File -FilePath $programsFile
            "-----------" + (" " * ($maxLength - 8)) + "--" | Add-Content -Path $programsFile
            foreach ($prog in $windowsPrograms) {
                "$($prog.Name)" + (" " * (($maxLength - $prog.Name.Length) + 3)) + "$($prog.Id)" | Add-Content -Path $programsFile
            }

            Write-Host (Get-Translation "ProgramsListed2") -ForegroundColor Green
            Pause   
        }
    } 
    catch {

        Write-Host (Get-Translation "WingetError") -ForegroundColor Red
        Write-Host (Get-Translation "WingetError2") -ForegroundColor Yellow
        Pause
        # return $null yerine döngüye geri dönmek için bir mekanizma ekleyin
    }
}
...
function Install-Programs {
    ...
    if (-not (Test-Path $Script:backupFolder)) {
        Write-Host (Get-Translation "BackUpNotFound") -ForegroundColor Red
        Pause
    }

    $programsFile = Join-Path $Script:backupFolder "InstalledPrograms.txt"
    $wingetCommand = Get-Command Get-WinGetPackage
    
    if (-not (Test-Path $programsFile)) {
        Write-Host (Get-Translation "InstalledProgramsNotFound") -ForegroundColor Red
        Pause
    }
    ...
    #maxLegngth değişkenini tekrar hesaplıyoruz.
    if (Get-Command Get-WinGetPackage -ErrorAction Stop){
        $windowsPrograms = Get-WinGetPackage | Select-Object Name , Id
        $maxLength = 0
        foreach($prog in $windowsPrograms) {
    ...
    #görüntülemek için programlistesi oluşturuyoruz.
    $lines = Get-Content $programsFile | Select-Object -Skip 2  
    $programList = @()
    foreach ($line in $lines) {
        
        if (-not [string]::IsNullOrWhiteSpace($line)) {
            
            $name= $line.Substring(0, $maxLength).Trim()
            $id= $line.Substring($maxLength).Trim()
    ...
    $itemsPerPage = 10
    $currentPage = 0
    $totalPages = [math]::Ceiling($programList.Count / $itemsPerPage)
    $selectedApps = New-Object System.Collections.ArrayList
    ...
    do {
        ...
        Write-Host "`n" $padding "Program List" "$($currentPage + 1) of $totalPages`n" -ForegroundColor Cyan
    
        
        $startIndex = $currentPage * $itemsPerPage
        $endIndex = [Math]::Min($startIndex + $itemsPerPage, $programList.Count)

        for ($i = $startIndex; $i -lt $endIndex; $i++) {
            $isSelected = $selectedApps.Contains($i)
            $marker = if ($isSelected) { "[X]" } else { "[ ]" }
            Write-Host $padding "$marker [$i] $($programList[$i].Name)" -ForegroundColor $(if ($isSelected) { "Green" } else { "White" })
        }
        ...
        Write-Host $padding "[A] " (Get-Translation 'SelectAllOnPage') -ForegroundColor Yellow
        Write-Host $padding "[C] " (Get-Translation 'ClearAllSelections') -ForegroundColor Yellow
        Write-Host $padding "[i] " (Get-Translation 'InstallSelectedPrograms') -ForegroundColor Green
        ...
        switch ($choice) {
            ...
            "İ" { 
                if ($selectedApps.Count -gt 0) {
                    Write-Host "`n " $padding (Get-Translation 'Installing') -ForegroundColor Yellow
                    foreach ($index in $selectedApps) {
                        $appId = $programList[$index].Id
                        $appName = $programList[$index].Name
                        Write-Host $padding (Get-Translation 'Installing2') ": $appName" -ForegroundColor Yellow
                        Install-WinGetPackage -Id $appId
                    }
                    $selectedApps.Clear()
                    Write-Host $padding (Get-Translation 'SelectedProgramsInstalled') -ForegroundColor Green
                    pause
                } else {
                    Write-Host "`n" $padding (Get-Translation 'SelectProgramsFirst') -ForegroundColor Red
                    pause
                }
            }
            "Q" { break }  # Döngüden çıkmak için break kullanın
...
try {
    ...
    do {
        ...
        switch ($selection) {
                    ...
                    switch ($choice) {
                        "1" {
                            ...
                            do {
                                ...
                                for ($i = $startIndex; $i -lt $endIndex; $i++) {Write-Host $padding "[$($i + 1)] $($services[$i])" -ForegroundColor White}
            ...
            "Q" { return }  # Döngüden çıkmak için break kullanın
            "q" { return }  # Döngüden çıkmak için break kullanın
            default { Write-Host "`n     [!] " $(Get-Translation 'InvalidSelection') -ForegroundColor Red; pause }               
        }
...
Catch {
    ...
    Write-Host "Lütfen hatayı kontrol edin ve tekrar deneyin." -ForegroundColor Yellow
}
